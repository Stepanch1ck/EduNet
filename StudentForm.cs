using MySql.Data.MySqlClient;
using System;
using System.Data;
using System.Windows.Forms;

namespace EduNet
{
    public partial class StudentForm : Form
    {
        private DataTable table = null;
        private MySqlDataAdapter adapter = null;
        public StudentForm()
        {
            InitializeComponent();
        }
        void ScheduleStudent_Click(object sender, EventArgs e)
        {
            editStudent.Visible = false;
            performancePanel.Visible = false;
            examPanel.Visible = false;
            schedulePanel.Visible = true;
            var dbc = new DBClass();
            dbc.OpenConnection();
            var query = "SELECT * FROM lesson";
            adapter = new MySqlDataAdapter(query, dbc.GetConnection());
            table = new DataTable();
            adapter.Fill(table);
            dataSchedule.Visible = true;
            dataSchedule.DataSource = table;
            dbc.CloseConnection();
        }

        void nameStudent_Click(object sender, EventArgs e)
        {
            performancePanel.Visible = false;
            examPanel.Visible = false;
            schedulePanel.Visible = false;
            editStudent.Visible = true;
        }

        void editSurname_Click(object sender, EventArgs e)
        {
            editSurname.Text = String.Empty;
        }

        void editName_Click(object sender, EventArgs e)
        {
            editName.Text = String.Empty;
        }

        void editPatronomyc_Click(object sender, EventArgs e)
        {
            editPatronomyc.Text = String.Empty;
        }

        void editEmail_Click(object sender, EventArgs e)
        {
            editEmail.Text = String.Empty;
        }

        void saveChanges_Click(object sender, EventArgs e)
        {
            var dbc = new DBClass();
            var mySqlCommand = new MySqlCommand("UPDATE student SET `surname` = @uS, `name` = @uN, `patronomyc` = @uP, `login` = @uL WHERE ID = @uI", dbc.GetConnection());
            var dataTable = new DataTable("student");
            mySqlCommand.Parameters.Add("@uS", MySqlDbType.VarChar).Value = editSurname.Text;
            mySqlCommand.Parameters.Add("@uN", MySqlDbType.VarChar).Value = editName.Text;
            mySqlCommand.Parameters.Add("@uP", MySqlDbType.VarChar).Value = editPatronomyc.Text;
            mySqlCommand.Parameters.Add("@uL", MySqlDbType.VarChar).Value = editEmail.Text;
            mySqlCommand.Parameters.Add("@uI", MySqlDbType.Int16).Value = 1;
            dbc.OpenConnection();
            if (mySqlCommand.ExecuteNonQuery() == 1)
            {
                MessageBox.Show("Вы успешно изменили профиль");
            }
            else
            {
                MessageBox.Show("Не удалось изменить профиль");
            }
            dbc.CloseConnection();
        }

        void performance_Click(object sender, EventArgs e)
        {
            editStudent.Visible = false;
            schedulePanel.Visible = false;
            examPanel.Visible = false;
            performancePanel.Visible = true;
            var dbc = new DBClass();
            dbc.OpenConnection();
            var query = "SELECT * FROM performance";
            adapter = new MySqlDataAdapter(query, dbc.GetConnection());
            table = new DataTable();
            adapter.Fill(table);
            dataPerformance.Visible = true;
            dataPerformance.DataSource = table;
            dbc.CloseConnection();
        }

        void exams_Click(object sender, EventArgs e)
        {
            performancePanel.Visible = false;
            editStudent.Visible = false;
            schedulePanel.Visible = false;
            examPanel.Visible = true;
            var dbc = new DBClass();
            dbc.OpenConnection();
            var query = "SELECT * FROM exam";
            adapter = new MySqlDataAdapter(query, dbc.GetConnection());
            table = new DataTable();
            adapter.Fill(table);
            dataExam.Visible = true;
            dataExam.DataSource = table;
            dbc.CloseConnection();
        }
    }
}
